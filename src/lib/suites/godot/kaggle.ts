...
/**
 * Log a user in to HuggingFace before anything downloads from it.
 *
 * Pixal3D's own background remover was gated on HuggingFace and was only
 * discovered because a run got five minutes in and died with "You are trying
 * to access a gated repo" — a failure that costs the run and says nothing
 * about the fix being "log in". Any dependency this pipeline picks up next
 * that turns out to be gated fails the same way unless something is logged in
 * before install() runs.
 *
 * Embedded in the pushed source rather than kept out of it, which matches how
 * everything else here already works: the images in a batch job are inlined
 * as base64 in the same source, and the kernel this goes to is pushed with
 * isPrivate: true — someone's game assets are not a public notebook, and
 * neither is their token.
 *
 * huggingface_hub reads HF_TOKEN itself, so setting the environment variable
 * is the whole job — no login() call, no huggingface-cli, nothing that could
 * be a different version away from working.
 */
function hfTokenPreamble(token?: string): string {
  if (!token?.trim()) return '';
  return `
HF_TOKEN = ${JSON.stringify(token.trim())}
os.environ["HF_TOKEN"] = HF_TOKEN
os.environ["HUGGING_FACE_HUB_TOKEN"] = HF_TOKEN  # older huggingface_hub reads this name
print("HuggingFace token present: gated downloads will authenticate.", flush=True)
`;
}
...
${PREAMBLE}${hfTokenPreamble(options.huggingfaceToken)}
...
export function pixal3dNotebook(
  ...
  options: { cached?: boolean; huggingfaceToken?: string } = {},
): string {
...
export async function generateOnKaggle(
  ...
  options: {
    ...
    signal?: AbortSignal;
    /**
     * Logs the run in to HuggingFace before install() runs, so a dependency
     * that turns out to be gated is a normal download instead of a 401 five
     * minutes into a GPU booking — which is exactly how the background
     * remover's own gating was first discovered.
     */
    huggingfaceToken?: string;
  },
  ...
  const pushed = await pushKernel(token, {
    ...
    source: pixal3dNotebook(jobs, options.toBase64, { cached: haveCache, huggingfaceToken: options.huggingfaceToken }),

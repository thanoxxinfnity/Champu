...
/** The install, shared by the setup run and the generate run. */
function installScript(withCachedWheel: boolean): string {
  return `
def install():
...
${
    withCachedWheel
      ? `    # The wheel built by a previous run, attached as an input. Ten seconds
    # instead of the fifteen-minute CUDA compile it replaces.
    wheels = glob.glob("/kaggle/input/**/natten-*.whl", recursive=True)
    if wheels:
        print("using cached natten wheel:", wheels[0], flush=True)
        sh("pip install -q --no-deps " + wheels[0])
    else:
        print("no cached wheel — compiling natten, which is the slow part", flush=True)
...
export function setupNotebook(): string {
  return `# Generated by Chomugiri. Builds the one thing that has to be compiled.
${PREAMBLE}
# A wheel, not an install: it is the artifact later runs attach.
sh("NATTEN_CUDA_ARCH='${PIXAL3D.cudaArch}' NATTEN_N_WORKERS=2 pip wheel natten==0.21.0 --no-build-isolation --no-deps -w /kaggle/working")

built = glob.glob("/kaggle/working/natten-*.whl")
print("CHOMUGIRI_RESULT", json.dumps({
    "models": [],
    "failed": [] if built else [{"name": "natten", "why": "the wheel did not build"}],
    "wheels": [os.path.basename(w) for w in built],
...
${installScript(options.cachedWheel ?? false)}
install()
...
export async function generateOnKaggle(
  ...
  const pushed = await pushKernel(token, {
    ...
    source: pixal3dNotebook(jobs, options.toBase64),
    gpu: true,
    ...(options.signal ? { signal: options.signal } : {}),
  });

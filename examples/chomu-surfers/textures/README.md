...
Microsoft FLUX.1-dev on NVIDIA NIM when it builds the game; the plan and the
prompts live in `src/lib/suites/godot/textures.ts`.
...
to the flat colours, which is why those colours are still in the code.
